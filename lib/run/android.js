...
function prepareAndroid({options}) {
    return new Promise((resolve, reject) => {
        ...
        process.chdir(path.join(rootPath, 'platforms/android/WeexWeiui'));
...
function runApp({device, options}) {
    return new Promise((resolve, reject) => {
        ...
        try {
            child_process.execSync(`adb -s ${device} shell am start -n ${packageName}/cc.weiui.app.WelcomeActivity`, {encoding: 'utf8'})
        } catch (e) {
            reject(e)
        }
        resolve()
