...
module.exports = {
    ...
    copySrcToDist() {
            ...
            lists.forEach((item) => {
                ...
                fs.stat(originPath, (err, stats) => {
                    if (typeof stats === 'object') {
                        if (stats.isFile()) {
                            if (/(\.(png|jpe?g|gif|ttf|svg|json|js|css|html|htm)$)/.test(originPath)) {
        ...
        _copyEvent(path.resolve(this.sourceDir), path.resolve(this.distDir));
    },
    ...
    syncFolderEvent(host, port, socketPort, removeBundlejs) {
        ...
        let hostUrl = 'http://' + host + ':' + port + '/' + this.distPages;
        //
                        ...
                        if (typeof stats === 'object' && stats.isDirectory()) {
                            if (removeBundlejs) {
                                fsEx.remove(path.resolve(assetsPath), (err) => {
                                    ...
                                    copyJsEvent(path.resolve(this.distDir), path.resolve(assetsPath));
                                });
                            }else{
                                copyJsEvent(path.resolve(this.distDir), path.resolve(assetsPath));
                            }
                        ...
                        if (typeof stats === 'object' && stats.isDirectory()) {
                            if (removeBundlejs) {
                                fsEx.remove(path.resolve(bundlejsPath), (err) => {
                                    ...
                                    copyJsEvent(path.resolve(this.distDir), path.resolve(bundlejsPath));
                                });
                            }else{
                                copyJsEvent(path.resolve(this.distDir), path.resolve(bundlejsPath));
                            }
    ...
    dev() {
        ...
        fsEx.removeSync(path.resolve(this.distDir));
        //
        let serverStatus = 0;
        ...
        return new WeiuiBuilder(this.sourcePages, this.distPages, options).build(callback);
    },
