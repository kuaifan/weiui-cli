...
function add(op) {
    ...
    path += dirCut + 'plugins' + dirCut + 'android' + dirCut + op.name;
...
function download(op) {
    ...
    outputPath += dirCut + 'plugins' + dirCut + 'android' + dirCut + op.name;
    //
    ...
    //
    if (util.count(op.android_lists) <= 1 || op.simple === true) {
...
function changeSetting(op, add) {
    let path = process.cwd();
    path += dirCut + 'platforms' + dirCut + 'android' + dirCut + 'WeexWeiui' + dirCut + 'settings.gradle';
...
function changeGradle(op, add) {
    let path = process.cwd();
    path += dirCut + 'platforms' + dirCut + 'android' + dirCut + 'WeexWeiui' + dirCut + 'app' + dirCut + 'build.gradle';
    let result = fs.readFileSync(path, 'utf8');
