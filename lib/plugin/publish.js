 ...
 */
function publish(op) {
    op.androidPath = process.cwd() + '/plugins/android/' + op.name;
    op.iosPath = process.cwd() + '/plugins/ios/' + op.name;
    ...
    op.androidZipPath = op.androidPath + '/' + op.name + '_android.zip';
    op.iosZipPath = op.iosPath + '/' + op.name + '_ios.zip';
    //
    inquirer.prompt([{
        type: 'input',
        name: 'desc',
...
function launch(op) {
    ...
    request.post({
        ...
        log.weiuis('插件' + op.name + '发布成功！');
        log.weiui('开发者可通过 ' + chalk.white(`weiui plugin install ${op.name}`) + ' 命令添加此插件。');
        //
