...
function create(op) {
    inquirer.prompt([{
        ...
        let androidPath = process.cwd() + '/plugins/android/' + op.name;
        let iosPath = process.cwd() + '/plugins/ios/' + op.name;
...
function createProject(projectType, projectName, projectPath) {
    ...
    if (projectType === "android") {
        demoPath = __dirname + "/template/android/PluginDemo";
    } else if (projectType === "ios") {
        demoPath = __dirname + "/template/ios/PluginDemo";
    } else {
        return;
    }
    ...
    for (let index in lists.dir) {
        ...
        let newPath = oldPath.replace(new RegExp(demoPath, "g"), projectPath);
        newPath = newPath.replace(new RegExp(demoName, "g"), projectName);
    ...
    for (let index in lists.file) {
        ...
        let result = fs.readFileSync(oldPath, 'utf8').replace(new RegExp(demoName, "gm"), projectName);
        if (result) {
