...
const backupMethod = {
    backup(callback) {
        let startBack = () => {
            ...
            //备份文件
            let dirName = !fs.existsSync(path.resolve("platforms/android/WeiuiApp/build.gradle")) ? "WeexWeiui" : "WeiuiApp";
            let dirLists = [
                "/src",
            ...
            for (let i = 0; i < dirLists.length; i++) {
                ...
                for (let index in lists.dir) {
                    ...
                    let saveName = tmp.substr(count).replace(new RegExp(dirName, "g"), "WeiuiApp");
            ...
            archive.file(process.cwd() + "/weiui.config.js", {name: "weiui.config.bak"});
            //完成备份
            output.on('close', () => {
    ...
    recoveryHandler(zipFile, callback) {
        fs.createReadStream(zipFile).pipe(unzip2.Extract({path: process.cwd()})).on('error', (err) => {
            ...
            let configBakPath = process.cwd() + '/weiui.config.bak';
            let intervalNum = 0;
